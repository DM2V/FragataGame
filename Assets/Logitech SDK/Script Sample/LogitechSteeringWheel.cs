...
public class LogitechSteeringWheel : MonoBehaviour
{
    ...
    Rigidbody rb; // Reference to the Rigidbody component of your entity
    public float maxAccelerationForce = 10.0f; // Adjust the force to your liking
    public float maxAccelerationForceHorizontal = 30.0f; // Adjust the force to your liking
    private Transform spriteTransform; // Store a reference to the sprite's transform�
    private Transform cameraTransfrom;
    private Quaternion initialRotation; // Store the initial rotation of the sprite
    public float maxVelocityForward = 20.0f;
    public float maxVelocityHorizontal = 10.0f;
    public float horizontalForwardMagnitude = 5.0f;
    ...
    void Start()
    {
        ...
        spriteTransform = GameObject.Find("HMS_Daring_Type_45").transform; // Replace with your sprite's GameObject name
        initialRotation = spriteTransform.rotation;
        cameraTransfrom = GameObject.Find("Camera").transform; // Replace with your sprite's GameObject name
    ...
    private float getNormalizedForce(int maxState, int minState, int currentState, float maxAcc, float minAcc)
    {
        float normalizedValue = Mathf.InverseLerp(maxState, minState, currentState);


        return Mathf.Lerp(minAcc, maxAcc, normalizedValue);
    }

    private void handleGasPress(int currentState)
    {
        int UP_STATE = 32767;
        int DOWN_STATE = -UP_STATE;

        float normalizedValue = Mathf.InverseLerp(UP_STATE, DOWN_STATE, currentState);
        float gasPedalForce = Mathf.Lerp(0, maxAccelerationForce, normalizedValue);

        Vector3 forwardForce = transform.forward * -gasPedalForce;
        rb.AddForce(forwardForce);
        rb.velocity = Vector3.ClampMagnitude(rb.velocity, maxVelocityForward);

    }

    private void handleWheelMovement(int currentState)
    {
        const int LEFT_MAX_STATE = -32768;
        const int RIGHT_MAX_STATE = (-LEFT_MAX_STATE) - 1;



        float normalizedValue = Mathf.InverseLerp(RIGHT_MAX_STATE, LEFT_MAX_STATE, currentState);
        float wheelMomentForce = Mathf.Lerp(-maxAccelerationForceHorizontal, maxAccelerationForceHorizontal, normalizedValue);

        // Calculate the rotation angle based on the wheelMomentForce
        float rotationAngle = Mathf.Lerp(15f, -15f, Mathf.InverseLerp(-maxAccelerationForceHorizontal, maxAccelerationForceHorizontal, wheelMomentForce));

        // Rotate the sprite to match the angle
        Quaternion targetRotation = Quaternion.Euler(0, rotationAngle, 0);
        spriteTransform.rotation = Quaternion.RotateTowards(spriteTransform.rotation, targetRotation, Time.deltaTime * 2);
        cameraTransfrom.rotation = Quaternion.RotateTowards(cameraTransfrom.rotation, targetRotation, Time.deltaTime * 2);
    }
